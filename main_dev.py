...
def Handle_Entity_Links(url):
    ...
    try:
        ...
        if r.status_code == 200:
            ...
            # if status_list:
                # span_text = status_list[0] not in FULL_TEXTS
                # if span_text:
            title = re.findall(
                    TITLE_REGEX,
...
Members_Bot = CHOOSE_FILTER_PRIVATE & filters.TEXT & filters.SenderChat(863875519)
...
app.add_handler(MessageHandler(filters.TEXT & Members_Bot & filters.Regex(PATTERN_TESTFLIGHT), Handle_TestflightApps_Private))
app.add_handler(MessageHandler(filters.TEXT & (filters.Entity("url") | filters.Entity("text_link")) & Members_Bot, Handle_TestflightApps_Entities))
